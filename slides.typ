...
#slide[
  ...
  - It uses the concepts of ownership and lifetimes to ensure memory safety at compile time, without the need for a garbage collector 
...
#slide[
  == Research Questions
  1. What development friction exist for developers working on systems programming projects, such as the linux kernel?
  2. How does Rust's language design effect the cognitive load of developers in systems programming?
  3. How does the workflow (use of tools for common task) differ between the two langauges?
  #pdfpc.speaker-note(```md
...
#slide[
  ...
  Rust's type system allows for moving more verification of correct implementation to compile time
  - This is because the type system can be used to enforce invariants that would otherwise be checked at runtime
  - While this example is related embedded systems, this was a common theme for papers exploring using rust for drivers and network protocols
  ```rs
...
#slide[
  ...
  The length of feedback loops is one of the three core dimensions of developer experience
  - Rust's compile times tend to be a source of frustration for larger projects
  - compile times suffer when making heavy use of generics, user defined types and macros
  #pdfpc.speaker-note(```md
...
#slide[
  ...
  - in rust, there is a single tool for most tasks, and the community tends to create subcommands for tools related to rust development
//   #table(
...
#slide[
  == 
  the effect is there seems a higher level of integration between tools in rust, and there is more focus on code reuse
  - all dependencies are at least somewhat documented.
  - it's easy to publish a crate, and at the moment there are 132,254 published crates(binaries and libraries)
...
#slide[
  ...
  - The general takeaway from papers regarding C is that it's difficult to write correct code, and it's difficult to onboard new developers. Prerequisite knowledge and experience is often irreplaceable
  
  - Rust has a higher initial learning curve, but it allows for moving more verification of correct implementation to compile time
...
#slide[
  ...
  - Does Rust's built in testing and documentation lead to developers writing more tests and documentation?
  - For developers who have experience with both, does rust's tooling lead to a higher level of productivity?
  - I wonder how the "information density per expression" of rust compares to C and C++
  - Is there a way to measure the differences in complexity in ported or replaced components?
  #pdfpc.speaker-note(```md
...
#slide[
  ...
  numbers.
  - This is useful because it allows us to convert a polynomial from one representation to the other in $O(n log n)$ time.
  - Computing the FFT of a vector of coefficients gives us the points of the polynomial, and computing the inverse FFT of a set of points gives us the coefficients of the polynomial.
  #pdfpc.speaker-note(```md
...
#slide[
  ...
  - When used in signal processing, it can break down a signal into its constituent frequencies. These can then be filtered out or modified and the signal can be reconstructed.
  - Because of this it's used heavily in audio processing, image compression, radar, sonar, seismology, and more.
...
#slide[
  ...
  - Attribution for the Fourier Transform is often given to Jean-Baptiste Joseph Fourier, in a paper published in 1822.
  - The Discrete Fourier Transform was first defined in 1805 by Carl Friedrich Gauss, but it was not published until 1866.
...
#centered-slide[
  The Fast Fourier Transform was popularized by James Cooley and John Tukey in 1965, but it was first described by Carl Friedrich Gauss in 1805.
  #pdfpc.speaker-note(```md
...
#slide[
  == FFT Implementations
  Given that which algorithm is optimal is often context dependent, Libraries that provide FFT implementations often employ a planner that chooses the best (hybrid) algorithm for the given input and the hardware. @frigoDesignImplementationFFTW32005

  - The plan is cached so that the next time the same input is given, the     algorithm can be immediately executed.
